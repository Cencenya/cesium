...
import PlotFactory from "../../TextMapPlot/PlotFactory"
import PlotTypes from "../../TextMapPlot/PlotTypes"

export default class TextMapPlotLayer extends LayerBase {
    constructor(viewer) {
        super(viewer);
        this.initEvent();
    ...
    flyToByPlotCode(plotCode) {
        ...
        switch (plot.properties.plotType) {
            ...
            case PlotTypes.POLYGON:
                this.viewer.flyTo(plot.polygonEntity);
                break;
        }
    ...
    initEvent() {
        new Cesium.ScreenSpaceEventHandler(this.viewer.scene.canvas)
            .setInputAction(e => {
                ...
                // 拾取到对象 判断拾取到的对象类型
                if (!id.id || !id.id.type || id.id.type != "TextMapPlot") {
                    if (this.selectedPlot) {
                        this.selectedPlot.setSelected(false);
                        this.selectedPlot = undefined;
                        //触发选中事件
