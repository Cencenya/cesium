 ...
 export default class TextMapPlotEditor {
     ...
     initLeftClickEventHandler() {
         this.eventHandler.setInputAction(e => {
             ...
             if (!id) {
                 this.handleEditTextMapPlot();
                 return; // 没有拾取到对象 直接返回 不做任何操作
             }

             // 拾取到对象 判断拾取到的对象类型
             if (!id.id || id.id.plotType != "TextMapPlot") {
                 this.clear();
                 return
             };
             //重复点击同一个对象
             if (this.editTextMapPlot && this.editTextMapPlot.plotCode == id.id.plotCode) return;
             //拾取到新的TextMapPlot对象
             this.handleEditTextMapPlot(); //处理上一个编辑对象
             this.handlePickTextMapPlot(id.id);
     ...
     handleEditTextMapPlot() {
         this.clear();
         this.editTextMapPlot = undefined;
     ...
     //处理拾取到的对象
     handlePickTextMapPlot(pickId) {
         this.editTextMapPlot = this.textMapPlotLayer.getByPlotCode(pickId.plotCode);
         if (!this.editTextMapPlot) return; //图层里面没有该对象 说明是在绘制的时候触发该事件
         this.isEditing = false;
         this.isEdited = false;
         this.editTextMapPlot.openEditMode(true);
     ...
     initLeftDownEventHandler() {
         this.eventHandler.setInputAction((e) => {
             ...
             if (id.id.type == "TextMapPlotEditVertex" || id.id.type == "TextMapPlotEditMove") {
                 this.isEditing = true;
                 ...
                 this.clearMidVertex();
             } else if (id.id.type == "TextMapPlotEditMidVertex") { //点击了中点 马上新建一个点到线上
     ...
     initLeftUpEventHandler() {
         this.eventHandler.setInputAction(((e) => {
             ...
             this.clearMidVertex();
             this.createMidVertex();
             if (this.editTextMapPlot.PlotType == "FencePlot") {
                 this.editTextMapPlot.initHeights();
             }
     ...
     initMouseMoveEventHandler() {
         this.eventHandler.setInputAction(((e) => {
             ...
             if (!this.isEditing) return;
             // 判断是整体平移还是节点平移
             if (this.editVertext.type == "TextMapPlotEditMove") {
     ...
     moveEntityByOffset(startPosition, endPosition) {
         ...
         let plotType = this.editTextMapPlot.properties.plotType;
     ...
     createEditVertex() {
         this.vertexEntities = [];
         let positions = this.editTextMapPlot.getPositions();
     ...
     //创建中点节点
     createMidVertex() {
         const plotType = this.editTextMapPlot.properties.plotType;
